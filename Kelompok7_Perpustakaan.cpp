...
kartu_anggota datai[100], jml_anggota;

struct datai_peminjaman // Menghitung jumlah peminjaman buku dan pengembalian buku
{
    int jumlah_pinjam;
    int jumlah_kembali;
};
datai_peminjaman juml[100];

struct datai_buku // Menampilkan nama dan kode buku yang nantinya akan dipinjam
{
    string nama_buku;
    string id_buku;
};
...
void menu_utama() // Tampilan awal saat user masuk ke program
{
    system("cls");
    cout << "~~~~~~~~~~~~~~~~~~~oOo~~~~~~~~~~~~~~~~~~~" << endl;
    cout << "| Selamat dataing di Perpustakaan Gulag! |" << endl;
    cout << "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" << endl;
    cout << "\n1. Daftar Buku" << endl;
...
void list_anggota() // Ibarat seperti menjadi administrator, nantinya akan menampilkan keseluruhan datai anggota serta jumlah buku yang dipinjam
{
    jml_anggota.jumlah_anggota += 0;
    ...
    if (strcmp(password, "12345") == 0 && strcmp(username, "admin") == 0) {
        if (jml_anggota.jumlah_anggota != 0)
        {
            ...
            for (i = 0; i < jumlah; i++)
            {
                cout << i + 1 << ". " << "Nama: " << datai[i].nama_anggota << endl;
                cout << "   Id: " << datai[i].id_anggota << endl;
...
void pinjam_buku() { // Untuk proses meminjam buku
    ...
    for (i = 0; i < jml_anggota.jumlah_anggota; i++) {
        if (input_id == datai[i].id_anggota) {
            system("cls");
            cout << "Selamat dataing, " << datai[i].nama_anggota << "! Berikut adalah daftar buku yang tersedia: \n" << endl;
            for (urutan = 0; urutan <= 7; urutan++) {
                cout << urutan + 1 << ". " << buku[urutan].nama_buku << endl;
                cout << "   Kode Buku: " << buku[urutan].id_buku << endl;
            }
    ...
    for (i = 0; i < jml_anggota.jumlah_anggota; i++) {
        if (input_id == datai[i].id_anggota) {
            if(juml[i].jumlah_pinjam != 0) {
                system("cls");
                cout << datai[i].nama_anggota << " : " << datai[i].id_anggota;
